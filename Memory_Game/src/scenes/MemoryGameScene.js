    ...
    createBox(){
        ...
        for(let row = 0; row < 3; row++){
            for(let col = 0; col < 3 ; col++){
                this.boxGroup.get(width* xPer,y, `tilesheet`, 7).setSize(64,32).setOffset(0, 32).setData(`itemType`, level[row][col])
                xPer += 0.25
            }
            xPer = 0.25
            y += 150
    ...
    checkForMatch(){
        ...
        this.time.delayedCall(1000, () => {
            ...
            if(this.matchesCount >= 4){
                this.player.active = false
				this.player.setVelocity(0, 0)
                this.add.text(this.halfWidth, this.halfHeight+250, `You Win !`, {fontSize : 60}).setOrigin(0.5)
